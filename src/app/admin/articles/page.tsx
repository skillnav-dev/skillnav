...
function buildPageUrl(
  page: number,
  params: { status: string; source: string; search: string },
) {
...
export default async function AdminArticlesPage({ searchParams }: PageProps) {
  await requireAdmin();
  const { status, source, search, page } =
  ...
  return (
    <div>
      ...
      <p className="mb-3 text-sm text-muted-foreground">
        {search || source ? `找到 ${total} 篇文章` : `共 ${total} 篇文章`}
      </p>
      ...
      {totalPages > 1 && (
        <div className="mt-6">
          <AdminPagination
            ...
            buildPageUrl={(p) => buildPageUrl(p, { status, source, search })}
          />
        </div>
      )}
    </div>
