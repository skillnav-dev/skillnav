...
```
06:15  [自动] 晨间采集完成 (UTC 22:15)
06:30  [自动] Daily Brief 生成 (UTC 22:30) → status=draft
07:45  [自动] 健康检查 → Slack 报告

07:30  [人工] 唯一审核窗口 (20-30 min)
       ├─ Daily Brief: 扫一眼 → approve
       ├─ Draft 文章: 选 2-3 篇 → published
       ├─ 低质量 → hidden
       └─ 分发: 复制各平台格式，发帖
08:00  [人工] 微信/掘金 发出（通勤高峰）
08:30  [人工] 知乎 发出（到工位刷资讯）
09:00  [人工] X 发出（美西晚间 + 亚洲早间交叉）
       ── 早间 SOP 结束 ──

12:00  [定时发] 小红书（午休高峰，内容早间准备好）

18:15  [自动] 午后采集完成 (UTC 10:15)
       ── 午后 draft 攒到次日早上一并审核，不做晚间二次审核 ──
```

> **设计理由**: 人工操作集中在 07:30-09:00 一个时段（~30 min），砍掉晚间二次审核。
> 分发时间按各平台读者高峰倒排：微信/掘金(08:00) > 知乎(08:30) > X(09:00) > 小红书(12:00)。
...
07:30  [人工] 早间 SOP 同常规日（Daily Brief + 文章审核 + 分发）
08:30  [人工] 额外: 审核周刊 → published + 周刊分发 (多 ~10 min)
