# Handoff — SkillNav
<!-- Generated by /checkpoint at 2026-03-08 session 19 -->

## Objective
中文开发者的 AI 智能体工具站（Skills · MCP · 实战资讯），当前阶段：UI/UX 重构 Phase 0-5 全部完成
...
### 内容战略 2.0 实施
1. **数据层 + 类型体系** — DB 迁移（content_tier/series/series_number）+ TypeScript 类型
2. **周刊生产工具链** — digest → compose → publish 三步命令行
3. **GitHub 导航页** — 方案需重建
...
## Verify
- `test -f src/components/shared/copy-button.tsx && echo OK` — OK
- `grep "formatNumber" src/lib/utils.ts` — 存在
- `test ! -f src/components/skills/skill-install.tsx && echo OK` — OK（已删除）
- `grep 'as.*"h1"' src/app/skills/page.tsx` — 存在
- `grep "即将推出" src/components/home/newsletter-cta.tsx` — 存在
- `grep "R_kgDORhTuFw" src/components/skills/skill-comments.tsx` — 存在（Giscus repoId）
- `grep "w-full sm:w-\[140px\]" src/components/skills/skills-toolbar.tsx` — 存在
- `npm run build` — 构建通过

## Modified Files (Session 19)
- `src/lib/utils.ts` — 新增 `formatNumber` 函数
- `src/components/shared/copy-button.tsx` — 新建，共享 CopyButton 组件
- `src/components/skills/skill-install.tsx` — 删除（废弃）
- `src/components/skills/skill-install-tabs.tsx` — 改用共享 CopyButton
- `src/components/shared/code-block.tsx` — 改用共享 CopyButton
- `src/components/skills/skill-card.tsx` — 改用共享 formatNumber
- `src/components/skills/skill-sidebar.tsx` — 改用共享 formatNumber
- `src/components/skills/skill-meta.tsx` — 改用共享 formatNumber
- `src/components/mcp/mcp-card.tsx` — 改用共享 formatNumber
- `src/components/shared/section-header.tsx` — 新增 `as` prop
- `src/app/skills/page.tsx` — SectionHeader as="h1"
- `src/app/articles/page.tsx` — SectionHeader as="h1"
- `src/app/mcp/page.tsx` — SectionHeader as="h1"
- `src/app/weekly/page.tsx` — SectionHeader as="h1"
- `src/components/home/newsletter-cta.tsx` — 改为"即将推出"静态提示
- `src/components/shared/inline-newsletter-cta.tsx` — 改为"即将推出"静态提示
- `src/components/skills/skill-comments.tsx` — 填入 Giscus repoId/categoryId
- `src/components/skills/skills-toolbar.tsx` — Select 移动端响应式
- `src/components/articles/articles-toolbar.tsx` — Select 移动端响应式
