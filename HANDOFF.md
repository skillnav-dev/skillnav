# Handoff — SkillNav
<!-- Generated by /checkpoint at 2026-03-08 session 20 -->
...
## Verify
- `test -f scripts/generate-weekly.mjs && echo OK` — OK
- `grep "generate:weekly" package.json` — 存在
- `test -f .github/workflows/generate-weekly.yml && echo OK` — OK
- `test -f docs/plans/weekly-pipeline.md && echo OK` — OK
- `grep "export async function callLLM" scripts/lib/llm.mjs` — 存在
- `npm run generate:weekly -- --dry-run --no-llm --week-of 2026-03-02 2>&1 | grep "Found"` — Found 14 articles
- `npm run build` — 构建通过

## Modified Files (Session 20)
- `scripts/generate-weekly.mjs` — 新建，周刊生成主脚本
- `scripts/lib/llm.mjs` — 导出 `callLLM` 函数
- `package.json` — 新增 `generate:weekly` script
- `.github/workflows/generate-weekly.yml` — 新建，周一定时生成
- `docs/plans/weekly-pipeline.md` — 新建，操作文档

## Document Inventory
| 文件 | 状态 | 行数 | 说明 |
|------|------|------|------|
| `HANDOFF.md` | 更新 | ~190 | 交接文档 |
...
| `docs/plans/weekly-pipeline.md` | 新建 | 184 | 周刊生成工具链操作文档 |
